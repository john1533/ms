...
public abstract class TopicDetail3GroupAdapter extends TopicDetail3CommonAdapter {
    ...
    public class UserPollTask extends AsyncTask<Void, Void, BaseResultModel<List<PollItemModel>>> {
        ...
        protected BaseResultModel<List<PollItemModel>> doInBackground(Void... params) {
            return TopicDetail3GroupAdapter.this.postsService.getUserPolls(TopicDetail3GroupAdapter.this.boardId, this.topicId, this.itemId);
        }
        ...
        protected void onPostExecute(BaseResultModel<List<PollItemModel>> result) {
            ...
            if (result.getRs() != 0) {
                ...
                TopicDetail3GroupAdapter.this.pollItemList = (List) result.getData();
                if (TopicDetail3GroupAdapter.this.pollModel != null) {
                    TopicDetail3GroupAdapter.this.pollModel.setPollItemList(TopicDetail3GroupAdapter.this.pollItemList);
                }
                TopicDetail3GroupAdapter.this.updatePollResultView(TopicDetail3GroupAdapter.this.pollItemList);
    ...
    public TopicDetail3GroupAdapter(Context context, List<Object> detailList) {
            ...
            if (pollItemList != null && !pollItemList.isEmpty()) {
                ...
                holder.getPollSubmitBtn().setOnClickListener(new OnClickListener() {
                    public void onClick(View arg0) {
                        if (!LoginHelper.doInterceptor(TopicDetail3GroupAdapter.this.context, null, null)) {
                            return;
                        }
                        if (TopicDetail3GroupAdapter.this.pollModel.getPollStatus() == 2) {
                            TopicDetail3GroupAdapter.this.publishPoll(postsModel, holder);
                        } else if (TopicDetail3GroupAdapter.this.pollModel.getPollStatus() == 3) {
                            MCToastUtils.toastByResName(TopicDetail3GroupAdapter.this.context.getApplicationContext(), "mc_forum_poll_no_perm");
                        } else if (TopicDetail3GroupAdapter.this.pollModel.getPollStatus() == 4) {
                            MCToastUtils.toastByResName(TopicDetail3GroupAdapter.this.context.getApplicationContext(), "mc_forum_poll_close");
                        }
                    }
                });
            }
        ...
        for (int i = 0; i < j; i++) {
            ...
            MCPollSelectBar pollSelectBar = new MCPollSelectBar(this.inflater, this.resource, pollItem, new SelectPollDelegate() {
                public void onItemSelect(View view, MCPollSelectBar selectBar, boolean select) {
                    if (select) {
                        if (!TopicDetail3GroupAdapter.this.selectedPollViewList.contains(selectBar)) {
                            TopicDetail3GroupAdapter.this.selectedPollViewList.add(selectBar);
                        }
                    } else if (TopicDetail3GroupAdapter.this.selectedPollViewList.contains(selectBar)) {
                        TopicDetail3GroupAdapter.this.selectedPollViewList.remove(selectBar);
                    }
                }
            }, this.context);
            ...
            holder.getActivitySubmitBtn().setOnClickListener(new OnClickListener() {
                public void onClick(View v) {
                    if (activityModel.getType().equals(FinalConstant.TIPS)) {
                        MCToastUtils.toast(TopicDetail3GroupAdapter.this.context.getApplicationContext(), activityModel.getDescription());
                    } else if (activityModel.getType().equals("login")) {
                        TopicDetail3GroupAdapter.this.context.startActivity(new Intent(TopicDetail3GroupAdapter.this.context, UserLoginActivity.class));
                    } else {
                        String urlString = TopicDetail3GroupAdapter.this.resource.getString("mc_discuz_base_request_url") + TopicDetail3GroupAdapter.this.resource.getString("mc_forum_topic_activity") + "&accessToken=" + TopicDetail3GroupAdapter.this.sDb.getAccessToken() + "&accessSecret=" + TopicDetail3GroupAdapter.this.sDb.getAccessSecret() + "&sdkVersion=" + BaseApiConstant.SDK_VERSION_VALUE + "&tid=" + topicId + "&act=" + activityModel.getType() + "&apphash=" + MCStringUtil.stringToMD5((System.currentTimeMillis() + "").substring(0, 5) + BaseApiConstant.AUTHKEY).substring(8, 16);
                        Intent intent = new Intent(TopicDetail3GroupAdapter.this.context, WebViewFragmentActivity.class);
                        intent.putExtra("webViewUrl", urlString);
                        TopicDetail3GroupAdapter.this.context.startActivity(intent);
                    }
                }
            });
        }
        ...
        if (isExistRate) {
            ...
            holder.getRateMoreText().setOnClickListener(new OnClickListener() {
                public void onClick(View arg0) {
                    ...
                    intent.putExtra("webViewUrl", TopicDetail3GroupAdapter.this.postsModel.getRateShowAllUrl());
                    TopicDetail3GroupAdapter.this.context.startActivity(intent);
                }
            });
            return;
