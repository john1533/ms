...
import com.mobcent.discuz.base.widget.WheelDialog.OnWheelChangedListener;
import com.mobcent.discuz.module.custom.widget.constant.CustomConstant;
import com.mobcent.lowest.base.utils.MCResource;
import java.util.LinkedList;
import java.util.List;

public class MCWheelView extends View {
    ...
    private Handler animationHandler = new Handler() {
        public void handleMessage(Message msg) {
            MCWheelView.this.scroller.computeScrollOffset();
            int currY = MCWheelView.this.scroller.getCurrY();
            int delta = MCWheelView.this.lastScrollY - currY;
            MCWheelView.this.lastScrollY = currY;
            if (delta != 0) {
                MCWheelView.this.doScroll(delta);
            }
            if (Math.abs(currY - MCWheelView.this.scroller.getFinalY()) < 1) {
                currY = MCWheelView.this.scroller.getFinalY();
                MCWheelView.this.scroller.forceFinished(true);
            }
            if (!MCWheelView.this.scroller.isFinished()) {
                MCWheelView.this.animationHandler.sendEmptyMessage(msg.what);
            } else if (msg.what == 0) {
                MCWheelView.this.justify();
            } else {
                MCWheelView.this.finishScrolling();
            }
        }
    };
    ...
    private SimpleOnGestureListener gestureListener = new SimpleOnGestureListener() {
        public boolean onDown(MotionEvent e) {
            if (!MCWheelView.this.isScrollingPerformed) {
                return false;
            }
            MCWheelView.this.scroller.forceFinished(true);
            MCWheelView.this.clearMessages();
            return true;
        }

        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            MCWheelView.this.startScrolling();
            MCWheelView.this.doScroll((int) (-distanceY));
            return true;
        }

        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            int minY;
            MCWheelView.this.lastScrollY = (MCWheelView.this.currentItem * MCWheelView.this.getItemHeight()) + MCWheelView.this.scrollingOffset;
            int maxY = MCWheelView.this.isCyclic ? Integer.MAX_VALUE : MCWheelView.this.adapter.getItemsCount() * MCWheelView.this.getItemHeight();
            if (MCWheelView.this.isCyclic) {
                minY = -maxY;
            } else {
                minY = 0;
            }
            MCWheelView.this.scroller.fling(0, MCWheelView.this.lastScrollY, 0, ((int) (-velocityY)) / 2, 0, 0, minY, maxY);
            MCWheelView.this.setNextMessage(0);
            return true;
        }
    };
    ...
    public interface OnWheelScrollListener {
        void onScrollingFinished(MCWheelView mCWheelView);

        void onScrollingStarted(MCWheelView mCWheelView);
    }

    public MCWheelView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        initData(context);
    }

    public MCWheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initData(context);
    }

    public MCWheelView(Context context) {
        super(context);
        initData(context);
    }
    ...
    private void initResourcesIfNecessary() {
        ...
        if (this.centerDrawable == null) {
            this.centerDrawable = this.mcResource.getDrawable("mc_forum_wheel_val");
        }
        ...
        setBackgroundResource(this.mcResource.getDrawableId("mc_forum_wheel_bg"));
    }
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        if (mode == 1073741824) {
            ...
            if (mode == ExploreByTouchHelper.INVALID_ID && widthSize < width) {
                width = widthSize;
                recalculate = true;
            }
        }
    ...
    private void createLayouts(int widthItems, int widthLabel) {
        ...
        if (this.itemsLayout == null || this.itemsLayout.getWidth() > widthItems) {
            this.itemsLayout = new StaticLayout(buildText(this.isScrollingPerformed), this.itemsPaint, widthItems, widthLabel > 0 ? Alignment.ALIGN_OPPOSITE : Alignment.ALIGN_CENTER, CustomConstant.RATIO_ONE_HEIGHT, 15.0f, false);
        } else {
        ...
        if (!this.isScrollingPerformed && (this.valueLayout == null || this.valueLayout.getWidth() > widthItems)) {
            ...
            this.valueLayout = new StaticLayout(text != null ? text : "", this.valuePaint, widthItems, widthLabel > 0 ? Alignment.ALIGN_OPPOSITE : Alignment.ALIGN_CENTER, CustomConstant.RATIO_ONE_HEIGHT, 15.0f, false);
        ...
        if (this.labelLayout == null || this.labelLayout.getWidth() > widthLabel) {
            this.labelLayout = new StaticLayout(this.label, this.valuePaint, widthLabel, Alignment.ALIGN_NORMAL, CustomConstant.RATIO_ONE_HEIGHT, 15.0f, false);
        } else {
...
//        this.valuePaint.setColor(VALUE_TEXT_COLOR);//TODO
        this.valuePaint.setColor(0xfff);
...
//        this.itemsPaint.setColor(-16777216);//TODO
        this.valuePaint.setColor(0xfff);
